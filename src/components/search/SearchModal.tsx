          ...
          {loading ? (
            ...
            results.map((anime) => (
              <Link
                ...
                href={`/anime/${anime.id}`}
                onClick={onClose}
                className="flex items-center gap-5 p-4 bg-slate-900/40 rounded-2xl border border-transparent hover:border-blue-500/50 hover:bg-slate-900/80 cursor-pointer transition-all group"
              >
